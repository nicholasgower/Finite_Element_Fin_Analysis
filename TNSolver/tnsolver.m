...
infile = strcat(basefilename ,'.inp');
fid = fopen(infile,'r');

global logfID
logfile = strcat(basefilename ,'.log');
...
outfile = strcat(basefilename ,'.out');
...
outfile = strcat(basefilename ,'_nd.csv');
fid = fopen(outfile,'wt');
if (fid < 0)
  error('TNSolver: Cannot open the output file: %s\n', outfile)
end
...
outfile = strcat(basefilename ,'_cond.csv');
fid = fopen(outfile,'wt');
if (fid < 0)
  error('TNSolver: Cannot open the output file: %s\n', outfile)
end
...
rstfile = strcat(basefilename ,'.rst');
